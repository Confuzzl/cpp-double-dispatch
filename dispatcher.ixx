...
struct Dispatcher<TypeRegistrar<Types...>, Return, A, std::tuple<AfterA...>, B,
  ...
  // using FuncType = std::function<Return(A &, AfterA..., B &, AfterB...)>;

  static constexpr auto NUM_TYPES = sizeof...(Types);
  FuncType table[NUM_TYPES][NUM_TYPES];

  // template <typename X, typename Y>
  // void registerFunction(Return (*func)(X &, AfterA..., Y &, AfterB...)) {
  ...
  //  table[x][y] = [func](A &a, AfterA... as, B &b, AfterB... bs) {
  ...
  void reg() {
    ...
    static constexpr auto x =
        Registrar::template getIndex<std::remove_cvref_t<X>>();
    static constexpr auto y =
        Registrar::template getIndex<std::remove_cvref_t<Y>>();
